...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Yq":
		...
		case "Set":
			...
			var expr string
			if inputArgs["expr"] != nil {
				err = json.Unmarshal([]byte(inputArgs["expr"]), &expr)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg expr", err))
				}
			}
			var yamlFilePath string
			if inputArgs["yamlFilePath"] != nil {
				err = json.Unmarshal([]byte(inputArgs["yamlFilePath"]), &yamlFilePath)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg yamlFilePath", err))
				}
			}
			return (*Yq).Set(&parent, expr, yamlFilePath), nil
		case "Get":
			var parent Yq
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Yq).Get(&parent, ctx, expr, yamlFilePath)
		case "WithDirectory":
			var parent Yq
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Yq).WithDirectory(&parent, source), nil
		case "State":
			var parent Yq
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
		...
		case "Container":
			...
			return (*Yq).Container(&parent), nil
		case "Shell":
			var parent Yq
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
		...
		case "":
			...
			return New(image, version, source), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithDescription("Yq runtime\n").
			WithObject(
				dag.TypeDef().WithObject("Yq").
					WithFunction(
						dag.Function("Set",
							dag.TypeDef().WithObject("Yq")).
							WithDescription("Edit a yaml file following the given expression").
							WithArg("expr", dag.TypeDef().WithKind(StringKind), FunctionWithArgOpts{Description: "The yq expression to execute"}).
							WithArg("yamlFilePath", dag.TypeDef().WithKind(StringKind), FunctionWithArgOpts{Description: "The yaml file path to edit"})).
					WithFunction(
						dag.Function("Get",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Fetch a value from a yaml file").
							WithArg("expr", dag.TypeDef().WithKind(StringKind), FunctionWithArgOpts{Description: "The yq expression to execute"}).
							WithArg("yamlFilePath", dag.TypeDef().WithKind(StringKind), FunctionWithArgOpts{Description: "The yaml file path to read"})).
					WithFunction(
						dag.Function("WithDirectory",
							dag.TypeDef().WithObject("Yq")).
							WithDescription("Override the source directory").
							WithArg("source", dag.TypeDef().WithObject("Directory"), FunctionWithArgOpts{Description: "The source where yaml files are stored"})).
					WithFunction(
						dag.Function("State",
							dag.TypeDef().WithObject("Directory")).
							WithDescription("Get the directory given to Yq")).
					WithFunction(
						dag.Function("Container",
							dag.TypeDef().WithObject("Container")).
							WithDescription("Get the yq container")).
					WithFunction(
						dag.Function("Shell",
							dag.TypeDef().WithObject("Terminal")).
							WithDescription("Open a shell in the current container")).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("Yq")).
							WithArg("image", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "The image to use for yq", DefaultValue: JSON("\"mikefarah/yq\"")}).
							WithArg("version", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "The version of the image to use", DefaultValue: JSON("\"4.35.2\"")}).
							WithArg("source", dag.TypeDef().WithObject("Directory"), FunctionWithArgOpts{Description: "The source where yaml files are stored"}))), nil
	default:
