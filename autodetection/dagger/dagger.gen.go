...
func (r NodeAnalyzer) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Matches    []string
		PkgJsonRep string
	}
	concrete.Matches = r.Matches
	concrete.PkgJsonRep = r.PkgJsonRep
	return json.Marshal(&concrete)
}

func (r *NodeAnalyzer) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Matches    []string
		PkgJsonRep string
	}
	...
	r.Matches = concrete.Matches
	r.PkgJsonRep = concrete.PkgJsonRep
	return nil
}

func (r OciAnalyzer) MarshalJSON() ([]byte, error) {
	var concrete struct {
		Matches []string
	}
	concrete.Matches = r.Matches
...
func (r *OciAnalyzer) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Matches []string
	}
	err := json.Unmarshal(bs, &concrete)
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Autodetection":
		...
		case "Node":
			...
			return (*Autodetection).Node(&parent, ctx, src, patternExclusions)
		case "Oci":
			var parent Autodetection
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Autodetection).Oci(&parent, ctx, src, patternExclusions)
		default:
	...
	case "NodeAnalyzer":
		...
		case "IsTest":
			...
			return (*NodeAnalyzer).IsTest(&parent), nil
		case "IsYarn":
			var parent NodeAnalyzer
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*NodeAnalyzer).IsYarn(&parent), nil
		case "IsNpm":
			var parent NodeAnalyzer
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*NodeAnalyzer).IsNpm(&parent), nil
		case "IsPackage":
			var parent NodeAnalyzer
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
		...
		case "GetEngineVersion":
			...
			return (*NodeAnalyzer).GetEngineVersion(&parent)
		case "GetVersion":
			var parent NodeAnalyzer
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*NodeAnalyzer).GetVersion(&parent)
		case "GetName":
			var parent NodeAnalyzer
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
	...
	case "OciAnalyzer":
		switch fnName {
		case "IsOci":
			var parent OciAnalyzer
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
				panic(fmt.Errorf("%s: %w", "failed to unmarshal parent object", err))
			}
			return (*OciAnalyzer).IsOci(&parent), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithDescription("Auto detect runtime information for a specific project\n").
			WithObject(
				dag.TypeDef().WithObject("Autodetection").
					WithFunction(
						dag.Function("Node",
							dag.TypeDef().WithObject("NodeAnalyzer")).
							WithDescription("Expose node auto dection runtime information").
							WithArg("src", dag.TypeDef().WithObject("Directory"), FunctionWithArgOpts{Description: "The path to the project to analyze"}).
							WithArg("patternExclusions", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)).WithOptional(true), FunctionWithArgOpts{Description: "Define patterns to exclude from the analysis"})).
					WithFunction(
						dag.Function("Oci",
							dag.TypeDef().WithObject("OciAnalyzer")).
							WithDescription("Expose OCI dection runtime information").
							WithArg("src", dag.TypeDef().WithObject("Directory"), FunctionWithArgOpts{Description: "The path to the project to analyze"}).
							WithArg("patternExclusions", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)).WithOptional(true), FunctionWithArgOpts{Description: "Define patterns to exclude from the analysis"}))).
			WithObject(
				dag.TypeDef().WithObject("NodeAnalyzer").
			...
			WithObject(
				dag.TypeDef().WithObject("OciAnalyzer").
					WithFunction(
						dag.Function("IsOci",
							dag.TypeDef().WithKind(BooleanKind))).
					WithField("Matches", dag.TypeDef().WithListOf(dag.TypeDef().WithKind(StringKind)))), nil
	default:
