...
func (r SecretManager) MarshalJSON() ([]byte, error) {
	var concrete struct{}
	return json.Marshal(&concrete)
}

func (r *SecretManager) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
...
func (r GcpSecretManager) MarshalJSON() ([]byte, error) {
	var concrete struct{}
	return json.Marshal(&concrete)
}

func (r *GcpSecretManager) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
	err := json.Unmarshal(bs, &concrete)
	if err != nil {
		return err
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "GcpSecretManager":
		...
		case "GetSecret":
			...
			var gcloudFolder *Directory
			if inputArgs["gcloudFolder"] != nil {
				err = json.Unmarshal([]byte(inputArgs["gcloudFolder"]), &gcloudFolder)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg gcloudFolder", err))
				}
			}
			return (*GcpSecretManager).GetSecret(&parent, ctx, name, project, version, filePath, gcloudFolder)
		case "SetSecret":
			var parent GcpSecretManager
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
	...
	case "":
		return dag.Module().
			WithDescription("Allow to create or read secret from GCP secret manager\n").
			WithObject(
				dag.TypeDef().WithObject("SecretManager").
					WithFunction(
						dag.Function("Gcp",
							dag.TypeDef().WithObject("GcpSecretManager")))).
			WithObject(
				dag.TypeDef().WithObject("GcpSecretManager").
					WithFunction(
						dag.Function("GetSecret",
							dag.TypeDef().WithObject("Secret")).
							WithDescription("Read a secret from secret manager").
							WithArg("name", dag.TypeDef().WithKind(StringKind), FunctionWithArgOpts{Description: "The secret name to read"}).
							WithArg("project", dag.TypeDef().WithKind(StringKind), FunctionWithArgOpts{Description: "The GCP project where the secret is stored"}).
							WithArg("version", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "The version of the secret to read", DefaultValue: JSON("\"latest\"")}).
							WithArg("filePath", dag.TypeDef().WithObject("File").WithOptional(true), FunctionWithArgOpts{Description: "The path to a credentials json file"}).
							WithArg("gcloudFolder", dag.TypeDef().WithObject("Directory").WithOptional(true), FunctionWithArgOpts{Description: "The path to the gcloud folder"})).
					WithFunction(
						dag.Function("SetSecret",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Create or update a secret value").
							WithArg("name", dag.TypeDef().WithKind(StringKind), FunctionWithArgOpts{Description: "The secret name to read"}).
							WithArg("value", dag.TypeDef().WithKind(StringKind), FunctionWithArgOpts{Description: "The value to set to the secret"}).
							WithArg("project", dag.TypeDef().WithKind(StringKind), FunctionWithArgOpts{Description: "The GCP project where the secret is stored"}).
							WithArg("filePath", dag.TypeDef().WithObject("File").WithOptional(true), FunctionWithArgOpts{Description: "The path to a credentials json file"}).
							WithArg("gcloudFolder", dag.TypeDef().WithObject("Directory").WithOptional(true), FunctionWithArgOpts{Description: "The path to the gcloud folder"}))), nil
	default:
